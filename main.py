...
def start_point(arr):
    '''Для работы волнового алгоритма ищем пустое место в игровом поле, и присваиваем ему 0'''
    flag = False
    for i in range(len(arr)):
        for j in range(len(arr)):
            if arr[i][j] == " ":
...
def wave(T):
    ...
    counter - число, равное количеству возможных шагов из каждой проверяемой пустой клетки'''
    counter = 0
    for i in range(len(arr)):
        for j in range(len(arr)):
            if arr[i][j] == str(T):
...
def wave_alg(arr):
    '''Волновой алгоритм
    Алгоритм проверяет путь по игровому полю
    Функция возвращает поле, в котором клетки с цифрами это пустые клетки игрового поля, * - игрок, $ - финиш, ' ' - преграда '''
    start_point(arr)
    T = 0
    while (True):
        if wave(T) == 0:
...
def possible(arr):
    ...
    Функция возвращает False после работы алгоритма чтобы сгенерировать поле в последующей функции'''
    for i in range(len(arr)):
        for j in range(len(arr)):
            if arr[i][j] == " ":
                return False
